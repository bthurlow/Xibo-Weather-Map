/**
 * File: c:\Projects\xibo-weather-map\web\modules\vendor\weather-map\scripts\owmleaflet.js
 * 
 * File Overview: A JavaScript library for using OpenWeatherMap's layers and OWM's city data for leaflet based maps without hassle.
 * 
 * Based on: https://github.com/buche/leaflet-openweathermap/
 * 
 * Project: scripts
 * 
 * Project Path: c:\Projects\xibo-weather-map\web\modules\vendor\weather-map\scripts
 * 
 * Created Date: Thursday, April 16th 2020, 8:43:20 am
 * 
...
L.OWM = L.TileLayer.extend({
    options: {
        ...
        apiVersion: 1,//available are 1, 2
        paidDev: false,// Is the appid on a Developer or higher paid plan?
        v1BaseUrl: "https://tile.openweathermap.org/map/{layername}/{z}/{x}/{y}.png",
        v1PaidBaseUrl: "https://maps.owm.io/map/{layer}/{z}/{x}/{y}.png?appid={api_key}",
        v2BaseUrl: "http://maps.openweathermap.org/maps/2.0/weather/{op}/{z}/{x}/{y}", //Has additional params that can be set, see https://openweathermap.org/api/weather-map-2.
        iconSet: 'default',//Available Options default,amchartsAnimated,amchartsStatic
        legendElement: null,//Use this var to store the legend as an element instead of an image.
    },
    initialize: function (options) {
        // console.log('L.OWM.initialize');
        // console.log(this);
        ...
        // console.log(this._v1LayerName);
        // console.log(typeof (this.newOpts));
        // console.log(this.options);
        // console.log(this.newOpts());

        // if (typeof (this.newOpts) != undefined) {
        //     options = { ...options, ...this.newOpts() };
        // }

        L.Util.setOptions(this, options);

        var tileurl = "";
        if (this.options.apiVersion === 1) {
            if (this.options.paidDev) {
                tileurl = this.options.v1PaidBaseUrl.replace('{layername}', this._v1LayerName);
            } else {
                tileurl = this.options.v1BaseUrl.replace('{layername}', this._v1LayerName);
            }
        } else {
            tileurl = this.options.v2BaseUrl.replace('{op}', this._v2LayerName);
        }
    ...
    onAdd: function (map) {
        ...
        if (this.options.showLegend && (this.options.legendImagePath != null || this.options.legendElement != null)) {
            this._legendControl = this._getLegendControl();
    ...
    _getLegendControl: function () {
        ...
        if (typeof this._map._owm_legendcontrol == 'undefined' || !this._map._owm_legendcontrol) {
            this._map._owm_legendcontrol = new L.OWM.LegendControl({
                ...
                img: this.options.legendImagePath,
                ele: this.options.legendElement
            });
...
// NOTE: Precipitation
/** 
 * OWM Precipitation Layer Extension
 *
 * @extends L.OWM
 */
 ...
 * 
 * Renders the precipitation layer
 *
 */
...
// NOTE: Radar
/** 
 * OWM Radar Layer Extension
 *
 * @extends L.OWM
 */
L.OWM.Radar = L.OWM.extend({
    _v1LayerName: 'radar',
    _v2LayerName: 'PR0'
});
/** 
...
//NOTE: Temperature
/** 
 * OWM Temperature Layer Extension
 *
 * @extends L.OWM
 */
...
L.OWM.temperature = (options) => {
    ...
    if (layer.options.unit === 'imperial') {
        ...
        layer.options.legendElement = '<div class="leaflet-control-color-scale leaflet-control" style="display: block; background: none; box-shadow: none; border-width: 0px; margin-right: 10px; width: 360px; height: 27px;"><div class="leaflet-control-color-scale-line" style="background-image: none; position: relative; border-width: 0px; margin: 0px;"><div class="scale-details"><div>Temperature, °C</div><div class="gradient-container" style="width: 260px;"><div class="scale-dividers"><div>-40</div><div>-20</div><div>0</div><div>20</div><div>40</div></div><div class="horizontal-gradient-line" style="background-image: linear-gradient(to right, rgb(159, 85, 181) 0%, rgb(44, 106, 187) 8.75%, rgb(82, 139, 213) 12.5%, rgb(103, 163, 222) 18.75%, rgb(142, 202, 240) 25%, rgb(155, 213, 244) 31.25%, rgb(172, 225, 253) 37.5%, rgb(194, 234, 255) 43.75%, rgb(255, 255, 208) 50%, rgb(254, 248, 174) 56.25%, rgb(254, 232, 146) 62.5%, rgb(254, 226, 112) 68.75%, rgb(253, 212, 97) 75%, rgb(244, 168, 94) 82.5%, rgb(244, 129, 89) 87.5%, rgb(244, 104, 89) 93.75%, rgb(244, 76, 73) 100%);"></div></div></div></div></div>';
    }
    return layer;
};
//NOTE: Wind Speed
/** 
 * OWM Wind Speed Layer Extension
 *
 * @extends L.OWM
 */
...
/** 
 * OWM Wind Speed Layer
 * 
 * Renders the wind speed layer
 *
 */
...
L.OWM.LegendControl = L.Control.extend({
    ...
    initialize: function (options) {
        // console.log('LegendControl Init');
        // console.log(options);
        // console.log(this.options);
    ...
    addLegend: function () {
        // console.log('LegendControl addLegend');
        // console.log(this.options);

        var legendId = this._legendCounter++;
        this._legendContainer[legendId] = this.options.img ? this.options.img : this.options.ele;
        this._redrawLegend();
    ...
    _redrawLegend: function () {
        ...
        for (var idx in this._legendContainer) {
            ...
            if (this.options.img) {
                ...
                item.innerHTML = this.options.ele;
            }
        }
    }
});
...
/** 
 * OWM Cities Extension
 *
 * @extends L.Layer
 */
L.OWM.Cities = L.Layer.extend({
    options: {
    ...
    initialize: function (options) {
        ...
        this._map = null;
        this._urlTemplate = 'https://api.openweathermap.org/data/2.5/box/city?{appId}cnt=300&format=json&units=metric&bbox={minlon},{minlat},{maxlon},{maxlat},10';
    ...
    _createPopup: function (station) {
        ...
        if (showLink) {
            ...
            txt += '<a href="https://openweathermap.org/' + typ + '/' + station.id + '" target="_blank" title="'
                + _i18n('owmlinktitle', 'Details at OpenWeatherMap', this.options.lang) + '">';
        }
        txt += station.name;
        if (showLink) {
            txt += '</a>';
        ...
        if (typeof station.main != 'undefined' && typeof station.main.temp != 'undefined') {
            txt += '<span class="owm-popup-temp">' + _temperatureConvert(station.main.temp, this.options.unit, this.options.temperatureDigits)
                + '&nbsp;' + _displayTemperatureUnit(this.options.unit) + '</span>';
        }
        txt += '</div>';
        ...
        if (typeof station.main != 'undefined') {
            if (typeof station.main.humidity != 'undefined') {
                txt += '<div class="owm-popup-detail">'
                    + _i18n('humidity', 'Humidity', this.options.lang)
                    + ': ' + station.main.humidity + '&nbsp;%</div>';
            }
            if (typeof station.main.pressure != 'undefined') {
                txt += '<div class="owm-popup-detail">'
                    + _i18n('pressure', 'Pressure', this.options.lang)
            ...
            if (this.options.showTempMinMax) {
                if (typeof station.main.temp_max != 'undefined' && typeof station.main.temp_min != 'undefined') {
                    txt += '<div class="owm-popup-detail">'
                        + _i18n('temp_minmax', 'Temp. min/max', this.options.lang)
                        + ': '
                        + _temperatureConvert(station.main.temp_min, this.options.unit, this.options.temperatureDigits)
                        + '&nbsp;/&nbsp;'
                        + _temperatureConvert(station.main.temp_max, this.options.unit, this.options.temperatureDigits)
                        + '&nbsp;' + _displayTemperatureUnit(this.options.unit) + '</div>';
                }
            }
        }
        if (station.rain != null && typeof station.rain != 'undefined' && typeof station.rain['1h'] != 'undefined') {
            txt += '<div class="owm-popup-detail">'
                + _i18n('rain_1h', 'Rain (1h)', this.options.lang)
        ...
        if (typeof station.wind != 'undefined') {
            if (typeof station.wind.speed != 'undefined') {
                ...
                if (this.options.showWindSpeed == 'beaufort' || this.options.showWindSpeed == 'both') {
                    txt += _i18n('windforce', 'Wind Force', this.options.lang)
                        + ': ' + _windMsToBft(station.wind.speed);
                    if (this.options.showWindSpeed == 'both') {
                        txt += '&nbsp;(' + _convertSpeed(station.wind.speed) + '&nbsp;'
                            + _displaySpeedUnit(this.options.unit) + ')';
                    }
                } else {
                    txt += _i18n('wind', 'Wind', this.options.lang) + ': '
                        + _convertSpeed(station.wind.speed, this.options.unit, this.options.speedDigits) + '&nbsp;'
                        + _displaySpeedUnit(this.options.unit);
                }
                txt += '</div>';
            }
            if (typeof station.wind.gust != 'undefined') {
                txt += '<div class="owm-popup-detail">';
                if (this.options.showWindSpeed == 'beaufort' || this.options.showWindSpeed == 'both') {
                    txt += _i18n('gust', 'Gust', this.options.lang)
                        + ': ' + _windMsToBft(station.wind.gust);
                    if (this.options.showWindSpeed == 'both') {
                        txt += '&nbsp;(' + _convertSpeed(station.wind.gust) + '&nbsp;'
                            + _displaySpeedUnit(this.options.unit) + ')';
                    }
                } else {
                    txt += _i18n('gust', 'Gust', this.options.lang) + ': '
                        + _convertSpeed(station.wind.gust, this.options.unit, this.options.speedDigits) + '&nbsp;'
                        + _displaySpeedUnit(this.options.unit);
                }
                txt += '</div>';
            }
            if (typeof station.wind.deg != 'undefined') {
                txt += '<div class="owm-popup-detail">';
                txt += _i18n('direction', 'Windrichtung', this.options.lang) + ': ';
        ...
        if (typeof station.dt != 'undefined' && this.options.showTimestamp) {
            ...
            txt += '(' + _convertTimestamp(station.dt, this.options.useLocalTime) + ')';
            txt += '</div>';
        }
        txt += '</div>';
        return txt;
    ...
    _getImageData: function (station) {
        ...
        if (station.weather && station.weather[0] && station.weather[0].icon) {
            ...
            imageUrl = _iconMap(this.options.iconSet, station.weather[0].icon, this.options.icons);
    ...
    _createMarker: function (station) {
        ...
        var icon = L.divIcon({
            className: ''
            , iconAnchor: new L.Point(imageData.width / 2, imageData.height / 2)
            // , popupAnchor: new L.Point(0, -10)
            // , html: this._icondivtext(station, imageData.url, imageData.width, imageData.height)
            , html: this._getOverlayTemplate(station, imageData.url)
        });
    ...
    //     if (typeof station.main != 'undefined' && typeof station.main.temp != 'undefined') {
    //         txt += '<div class="owm-icondiv-temp">' + _temperatureConvert(station.main.temp, this.options.unit, this.options.temperatureDigits)
    //             + '&nbsp;' + _displayTemperatureUnit(this.options.unit) + '</div>';
    //     }
    //     txt += '</div>';
    //     return txt;
    ...
    _getOverlayTemplate: function (station, imageurl) {
        ...
        switch (this.options.overlayTemplate) {
            case 1:
                txt += '<div class="owm-icondiv template1 container">';
                txt += '<div class="row"><div class="col col-sm-12 city"><div class="city-grad">' + station.name + '</div></div></div>';
                txt += '<div class="row">'
                    + '<div class="col-xs-4 icon"><img src="' + imageurl + '" border="0" width="50" height="50" /></div>'
                ...
                break;
            case 2:
                txt += '<div class="owm-icondiv template2 container">';
                txt += '<div class="row"><div class="col col-sm-12 city"><div class="city-grad">' + station.name + '</div></div></div>';
                txt += '<div class="row">'
                    + '<div class="col-xs-4" style="padding:2px;"><div class="icon" style="width:50px;"><img src="' + imageurl + '" border="0" width="50" height="50" /></div></div>'
                    + '<div class="col-xs-8 details"><div class="temp">' + _temperatureConvert(station.main.temp, this.options.unit, this.options.temperatureDigits) + _displayTemperatureUnit(this.options.unit) + '</div><div class="description">'
                    + _i18n('id' + station.weather[0].id, station.weather[0].description + ' (' + station.weather[0].id + ')', this.options.lang)
                    + '</div>'
                    + '</div>';
            ...
            default://1
                txt += '<div class="owm-icondiv">'
                    + '<img src="' + imageurl + '" border="0" width="50" height="50" />';
                if (typeof station.main != 'undefined' && typeof station.main.temp != 'undefined') {
                    txt += '<div class="owm-icondiv-temp">' + _temperatureConvert(station.main.temp, this.options.unit, this.options.temperatureDigits)
                        + '&nbsp;' + _displayTemperatureUnit(this.options.unit) + '</div>';
                }
...
L.OWM.cities = (options) => { return new L.OWM.Cities(options); };
...
L.OWM.Current = L.Layer.extend({
    ...
    _createPopup: function (station) {
        ...
        if (showLink) {
            ...
            txt += '<a href="https://openweathermap.org/' + typ + '/' + station.id + '" target="_blank" title="'
                + _i18n('owmlinktitle', 'Details at OpenWeatherMap') + '">';
        }
        txt += station.name;
        if (showLink) {
            txt += '</a>';
        ...
        if (typeof station.weather != 'undefined' && typeof station.weather[0] != 'undefined') {
            if (typeof station.weather[0].description != 'undefined' && typeof station.weather[0].id != 'undefined') {
                txt += '<div class="owm-popup-description">'
                    + _i18n('id' + station.weather[0].id, station.weather[0].description + ' (' + station.weather[0].id + ')')
                    + '</div>';
            }
        }
        var imgData = this._getImageData(station);
        ...
        if (typeof station.main != 'undefined') {
            if (typeof station.main.humidity != 'undefined') {
                txt += '<div class="owm-popup-detail">'
                    + _i18n('humidity', 'Humidity')
                    + ': ' + station.main.humidity + '&nbsp;%</div>';
            }
            if (typeof station.main.pressure != 'undefined') {
                txt += '<div class="owm-popup-detail">'
                    + _i18n('pressure', 'Pressure')
                    + ': ' + station.main.pressure + '&nbsp;hPa</div>';
            }
            if (this.options.showTempMinMax) {
                if (typeof station.main.temp_max != 'undefined' && typeof station.main.temp_min != 'undefined') {
                    txt += '<div class="owm-popup-detail">'
                        + _i18n('temp_minmax', 'Temp. min/max')
                        + ': '
        ...
        if (typeof station.wind != 'undefined') {
            if (typeof station.wind.speed != 'undefined') {
                txt += '<div class="owm-popup-detail">';
                if (this.options.showWindSpeed == 'beaufort' || this.options.showWindSpeed == 'both') {
                    txt += _i18n('windforce', 'Wind Force')
            ...
            if (typeof station.wind.deg != 'undefined') {
                txt += '<div class="owm-popup-detail">';
                txt += _i18n('direction', 'Windrichtung') + ': ';
                if (this.options.showWindDirection == 'desc' || this.options.showWindDirection == 'both') {
                    txt += this._directions[(station.wind.deg / 22.5).toFixed(0)];
                    if (this.options.showWindDirection == 'both') {
                        txt += '&nbsp;(' + station.wind.deg + '°)';
...
//Helper Functions
var _iconMap = (iconSet, condition, icons) => {
    // console.log('_iconMap');
    // console.log(icons);

    if (iconSet === 'default') {
        return "https://openweathermap.org/img/w/{icon}.png".replace('{icon}', condition);
    } else {
    ...
    //         url = icons[condition];
    //         // icon = amchartsAnimated.filter((v) => { return v.condition === condition; });
    //         // if (icon.length > 0) {
    //         //     url = "/library/" + icon[0].icon;
    //         // }
    //         break;
    //     case 'amchartsStatic':
    //         url = icons[condition];
    //         // icon = amchartsStatic.filter((v) => { return v.condition === condition; });
    //         // if (icon.length > 0) {
    //         //     url = "/library/" + icon[0].icon;
    //         // }
    //         break;
    //     default:
    //         url = "https://openweathermap.org/img/w/{icon}.png".replace('{icon}', condition);
    //         break;
    // }
    // return url;
};

var _temperatureConvert = (tempC, unit, digits) => {
    //console.log('Cities _temperatureConvert');
    //console.log(tempC);
...
var _displayTemperatureUnit = (unit) => {
    var tempUnits = { K: 'K', C: '&deg;C', F: '&deg;F' };
    var display = tempUnits['C'];
    switch (unit) {
        case 'metric':
            break;
        case 'imperial':
    ...
    // if (typeof tempUnits[this.options.temperatureUnit] != 'undefined') {
    //     unit = this._tempUnits[this.options.temperatureUnit];
    // }
    return display;
};
...
var _convertSpeed = (speed, unit, digits) => {
    var sp = speed;
    switch (unit) {
        case 'metric':
            sp = 3.6 * sp;
    ...
    return sp.toFixed(digits);
};

var _convertTimestamp = (tstmp, useLocalTime) => {
    if (useLocalTime) {
        return (new Date(tstmp * 1000));
    } else {
        return (new Date(tstmp * 1000)).toUTCString();
